classdef Dashboard_App_boost_cpu < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        VoutValueLabel              matlab.ui.control.Label
        ...
        StopTimeValueLabel          matlab.ui.control.Label

        LampLabel                   matlab.ui.control.Label
        RunningLamp                 matlab.ui.control.Lamp

        ClosedLoopNoteLabel         matlab.ui.control.Label
    ...
    properties (Access = private)
        MODEL_NAME = 'gmStateSpaceHDL_boost_convert_fixpt'
    end

    methods (Access = private)

        %--------------------------------------------------
        % Raccourci : chemin complet d'un bloc
        %--------------------------------------------------
        ...
        %--------------------------------------------------
        % Mettre à jour UN bloc Constant via set_param
        %--------------------------------------------------
        function setBlock(app, blockName, value)
            switch blockName
                case 'duty'
                    strVal = num2str(value, '%.6f');
                case 'freq'
                    strVal = num2str(value, '%.2f');
                case 'en'
                    strVal = num2str(uint32(value));
                case 'mode_controller'
                    strVal = num2str(uint32(value));
                case 'vout_ref'
                    strVal = num2str(value, '%.4f');
                case 'rst'
                    strVal = num2str(uint32(value));
                otherwise
        ...
        %--------------------------------------------------
        % Initialiser tous les blocs aux valeurs par défaut
        %--------------------------------------------------
        ...
        function updateDisplayedValues(app)
            vout_est = 5 / (1 - app.DutyKnob.Value);

            app.DutyValueLabel.Text     = sprintf('Duty = %.3f', app.DutyKnob.Value);
            app.VoutValueLabel.Text     = sprintf('Vout = %.2f V', vout_est);
            ...
            app.StopTimeValueLabel.Text = sprintf('Stop time = %.4f s', app.StopTimeField.Value);
        end
        ...
        function DutyKnobValueChanged(app, ~)
            v = max(0, min(0.7, app.DutyKnob.Value));
        ...
        function VrefSliderValueChanged(app, ~)
            v = max(5, min(10, app.VrefSlider.Value));
        ...
        function StopTimeFieldValueChanged(app, ~)
            if isempty(app.StopTimeField.Value) || app.StopTimeField.Value <= 0
                app.StopTimeField.Value = 0.05;
            end
            app.updateDisplayedValues();
        end
        ...
        function RunButtonPushed(app, ~)
            try
                ...
                stopT = app.StopTimeField.Value;
                if isempty(stopT) || stopT <= 0
                    stopT = 0.05;
                    app.StopTimeField.Value = stopT;
                end
                set_param(app.MODEL_NAME, 'StopTime', num2str(stopT, '%.12g'));
        ...
        function createComponents(app)
            ...
            app.UIFigure.Position = [100 100 980 780];
            app.UIFigure.Name = 'Control Dashboard Boost CPU';
            app.UIFigure.Color = [0.13 0.13 0.15];

            app.MainGrid = uigridlayout(app.UIFigure, [12 4]);
            app.MainGrid.RowHeight    = {40, 55, 28, 150, 28, 28, 50, 28, 40, 28, 30, 55};
            ...
            app.TitleLabel.Text = 'Control Dashboard Boost CPU';
            ...
            app.DutyKnob.Limits = [0 0.7];
            app.DutyKnob.MajorTicks = 0:0.1:0.7;
            ...
            % --- Row 6 : Vout calculé + Mode label ---
            app.VoutValueLabel = uilabel(app.MainGrid);
            app.VoutValueLabel.Text = 'Vout = 12(1 - Duty) = 6.00 V';
            app.VoutValueLabel.HorizontalAlignment = 'center';
            app.VoutValueLabel.FontSize = 12;
            app.VoutValueLabel.FontColor = [0.25 0.75 1.00];
            app.VoutValueLabel.Layout.Row = 6;
            app.VoutValueLabel.Layout.Column = 1;
            ...
            % --- Row 7 : Note rouge + ModeSwitch ---
            app.ClosedLoopNoteLabel = uilabel(app.MainGrid);
            app.ClosedLoopNoteLabel.Text = 'NOTE : Le contrôle en boucle fermée n''est pas fonctionnel pour le Boost converter.';
            app.ClosedLoopNoteLabel.HorizontalAlignment = 'left';
            app.ClosedLoopNoteLabel.FontSize = 12;
            app.ClosedLoopNoteLabel.FontWeight = 'bold';
            app.ClosedLoopNoteLabel.FontColor = [1.00 0.20 0.20];
            app.ClosedLoopNoteLabel.Layout.Row = 7;
            app.ClosedLoopNoteLabel.Layout.Column = [1 3];
            ...
            % --- Row 8 : Stop Time label ---
            app.StopTimeLabel = uilabel(app.MainGrid);
            app.StopTimeLabel.Text = 'Simulation Stop Time (s)';
            app.StopTimeLabel.HorizontalAlignment = 'center';
            app.StopTimeLabel.FontWeight = 'bold';
            app.StopTimeLabel.FontSize = 13;
            app.StopTimeLabel.FontColor = [0.9 0.9 0.9];
            app.StopTimeLabel.Layout.Row = 8;
            app.StopTimeLabel.Layout.Column = 3;

            % --- Row 9 : Stop Time field ---
            app.StopTimeField = uieditfield(app.MainGrid, 'numeric');
            app.StopTimeField.Value = 0.05;
            app.StopTimeField.Limits = [1e-6 Inf];
            app.StopTimeField.RoundFractionalValues = 'off';
            app.StopTimeField.HorizontalAlignment = 'center';
            app.StopTimeField.FontSize = 12;
            app.StopTimeField.ValueChangedFcn = createCallbackFcn(app, @StopTimeFieldValueChanged, true);
            app.StopTimeField.Layout.Row = 9;
            app.StopTimeField.Layout.Column = 3;

            % --- Row 10 : Stop Time value label ---
            app.StopTimeValueLabel = uilabel(app.MainGrid);
            app.StopTimeValueLabel.Text = 'Stop time = 0.0500 s';
            app.StopTimeValueLabel.HorizontalAlignment = 'center';
            app.StopTimeValueLabel.FontSize = 12;
            app.StopTimeValueLabel.FontColor = [0.35 0.80 0.35];
            app.StopTimeValueLabel.Layout.Row = 10;
            app.StopTimeValueLabel.Layout.Column = 3;

            % --- Row 11 : Label section Scopes ---
            ...
            app.ScopeSectionLabel.Layout.Row = 11;
            app.ScopeSectionLabel.Layout.Column = [1 4];

            % --- Row 12 : 3 boutons Scope ---
            ...
            app.ScopeOutputButton.Layout.Row = 12;
            ...
            app.ScopePWMButton.Layout.Row = 12;
            ...
            app.ScopeControllerButton.Layout.Row = 12;
    ...
    methods (Access = public)
        ...
        function app = Dashboard_App_boost_cpu
